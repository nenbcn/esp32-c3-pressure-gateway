...
void initializeWiFiConfigMode() {
    ...
    if (n > 0) {
        ...
        // Use simple duplicate detection without Vector library
        for (int i = 0; i < n; ++i)
        {
            String ssid = WiFi.SSID(i);
            
            // Check if SSID already added (simple string search)
            String searchPattern = "value='" + ssid + "'>";
            if (g_wifiOptions.indexOf(searchPattern) == -1)
            {
                g_wifiOptions += "<option value='" + ssid + "'>";
                g_wifiOptions += ssid;
                g_wifiOptions += "</option>";
            } else {
                Log::info("Found duplicate entry: %s", ssid.c_str());
            }
        }
    } else {
